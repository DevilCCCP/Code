...
bool DebugWnd::DoCircle()
{
  ...
  while (!mDrawList.isEmpty()) {
    ...
    lock.unlock();
    frameWnd->DrawWindow(info.Data, info.ImageType, info.Save, mWidth, mHeight, info.Stride);
    lock.relock();
  }
  return true;
}
...
void DebugWnd::DrawWindow(int index, const QString &caption, EImageType imageType, bool save, const char *data, int stride, const char *objData, int objSize)
{
#ifdef Q_OS_WIN32
  QMutexLocker lock(&mDrawMutex);
...
//    SystemParametersInfoA(SPI_GETWORKAREA, 0, &desktopRect, 0);
  ...
  if (index == 0) {
    lock.unlock();
    frameWnd->DrawWindow(data, imageType, save, mWidth, mHeight, stride, objData, objSize);
  } else {
